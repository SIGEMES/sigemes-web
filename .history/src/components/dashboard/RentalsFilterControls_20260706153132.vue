...
export default {
  ...
  props: {
    rentals: Array,
    paginatedRentals: Array,
    isLoading: Boolean,
    errorMessage: String,
    currentPage: Number,
    itemsPerPage: Number,
    totalPages: Number,
    loadingDetailId: [String, Number, null],
    // Props untuk applied filters dari parent
    appliedFilterAllDates: {
      type: Boolean,
      default: true,
    },
  ...
  emits: ['filters-updated', 'open-filter-modal', 'show-detail', 'page-changed', 'open-jump-modal'],
  data() {
    return {
      searchQuery: '',
      sortBy: 'newest',
      ...
      localItemsPerPage: 10, // Renamed untuk tidak conflict dengan props
  ...
  methods: {
    ...
    validateDateRange() {
      ...
      let isDateRangeEmpty = !this.dateRange || this.dateRange.length !== 2
    ...
    applyFilters() {
      ...
      // Emit filters ke parent (parent yang akan menyimpan applied filter state)
      const filtersToApply = {
        search: this.searchQuery,
        sort: this.sortBy,
        limit: this.localItemsPerPage,
        allDates: this.filterAllDates,
        startDate: this.filterStartDate,
        endDate: this.filterEndDate,
        rentalType: this.filterRentalType,
        paymentStatus: this.filterAllPaymentStatuses ? [] : [...this.selectedPaymentStatuses],
      }

      this.$emit('filters-updated', filtersToApply)
    ...
    selectSort(value) {
      this.sortBy = value
      this.showSortDropdown = false
      // Jangan reset applied filters, hanya emit perubahan sort
      this.emitFiltersWithoutReset()
    },

    selectItemsPerPage(value) {
      this.localItemsPerPage = value
      this.showItemsDropdown = false
      // Jangan reset applied filters, hanya emit perubahan limit
      this.emitFiltersWithoutReset()
    },
    ...
    handleSearch() {
      ...
      this.searchTimeout = setTimeout(() => {
        // Jangan reset applied filters, hanya emit perubahan search
        this.emitFiltersWithoutReset()
      }, 500)
    },

    openFilterModal() {
      // Sync modal filter values dengan applied filter values dari props parent
      ...
      // Sync payment status dari props parent
      ...
      // Sync date range untuk datepicker dari props parent
      ...
      // Emit event ke parent untuk membuka modal filter
      this.$emit('open-filter-modal')
    },

    emitFilters() {
      // Method ini tidak digunakan lagi karena applyFilters() langsung emit
      ...
      const activeFilters = {
        ...
        paymentStatus: this.appliedSelectedPaymentStatuses,
      }
      this.$emit('filters-updated', activeFilters)
    },

    // Method khusus untuk emit filter tanpa mereset applied filters
    // Digunakan ketika hanya mengubah search, sort, atau limit
    emitFiltersWithoutReset() {
      const activeFilters = {
        search: this.searchQuery,
        sort: this.sortBy,
        limit: this.localItemsPerPage,
        // Pertahankan filter yang sudah diterapkan dari props parent
        allDates: this.appliedFilterAllDates,
        startDate: this.appliedFilterStartDate,
        endDate: this.appliedFilterEndDate,
        rentalType: this.appliedFilterRentalType,
