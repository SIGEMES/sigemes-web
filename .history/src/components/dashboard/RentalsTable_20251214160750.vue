...
export default {
  ...
  data() {
    return {
      showInitialAnimation: true, // Flag untuk mengontrol animasi awal
  ...
  watch: {
    rentals: {
      handler(newVal) {
        // Jika data ada dan belum pernah animasi, aktifkan animasi
        if (newVal.length > 0 && !this.hasAnimated) {
          this.showInitialAnimation = true
          // Setelah animasi selesai (500ms), matikan animasi untuk interaksi selanjutnya
          setTimeout(() => {
            this.showInitialAnimation = false
            this.hasAnimated = true
          }, 600) // Sedikit lebih lama dari durasi animasi (500ms)
        }
      },
      immediate: true,
    },
