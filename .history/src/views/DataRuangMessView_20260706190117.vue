                        ...
                        <div v-else class="flex overflow-x-auto p-2 w-full h-full">
                          ...
                          >
                            <img
                              :src="preview"
                              class="w-32 h-32 object-cover rounded-lg cursor-pointer hover:opacity-80 transition opacity"
                              @click.stop="openAddRoomSlideshow(index)"
                            />
                            <button
                              @click.stop="removeRoomPhoto(index)"
                              class="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center"
...
export default {
  ...
  data() {
    return {
      ...
      showDeleteRoomModal: false, // State untuk modal konfirmasi hapus ruang
      roomToDelete: null, // Data ruang yang akan dihapus
      showDeleteGuesthouseModal: false, // State untuk modal konfirmasi hapus gedung mess
      guesthouseToDelete: null, // Data gedung mess yang akan dihapus
      isDeletingGuesthouse: false, // State loading untuk hapus gedung mess
      deletingGuesthouseId: null, // Track which guesthouse is being deleted

      // State tracking untuk perubahan
      hasUnsavedChanges: false, // Track apakah ada perubahan yang belum disimpan
      showCancelConfirmModal: false, // Modal konfirmasi cancel perubahan
      pendingDeletedRooms: [], // Array ID ruang yang akan dihapus (state sementara)
      pendingEditedRooms: [], // Array ruang yang diedit (state sementara)
      pendingNewRooms: [], // Array ruang baru yang ditambahkan (state sementara)
      originalEditRooms: [], // Backup data ruang asli untuk restore jika cancel
      ...
      // isi form awal ruang gedung mess - Array untuk multiple rooms
      newRooms: [], // Array untuk menyimpan semua ruang yang akan ditambahkan
      currentRoomIndex: null, // Index ruang yang sedang diedit
      showAddRoomModal: false, // State untuk modal tambah/edit ruang
      selectedAddRoomIndex: null, // Index ruang yang dipilih untuk edit (null = tambah baru)
      newRoom: {
        name: '',
        type: 'standard',
        facilities: '',
  ...
  methods: {
    // Validation methods
    validateDescription() {
      // Check both newGuesthouse and editGuesthouse
      const description = this.showAddGuesthouseModal
        ? this.newGuesthouse.description
        : this.editGuesthouse.description
    ...
    validateNumberOnly(event, field, type, index = null) {
      ...
      // Remove all non-numeric characters (termasuk titik pemisah ribuan)
      value = value.replace(/[^0-9]/g, '')

      // Preserve leading zeros for contact_person (phone numbers may start with 0)
      if (field !== 'contact_person') {
        // Prevent leading zeros for numeric fields (kecuali input adalah "0" saja)
        if (value.length > 1 && value.startsWith('0')) {
          value = value.replace(/^0+/, '')
        }
      }

      // Update the model based on type
      if (type === 'guesthouse') {
        if (this.showAddGuesthouseModal) {
          this.newGuesthouse[field] = value
        } else if (this.showEditGuesthouseModal) {
          this.editGuesthouse[field] = value
        }
      } else if (type === 'room') {
        if (this.showAddGuesthouseModal || this.showAddRoomModal) {
          this.newRoom[field] = value
        } else if (this.showEditGuesthouseModal) {
          this.editRoom[field] = value
        }
      } else if (type === 'pricing' && index !== null) {
        if (this.showAddGuesthouseModal || this.showAddRoomModal) {
          this.newRoom.pricing[index][field] = value
        } else if (this.showEditGuesthouseModal) {
          this.editRoom.pricing[index][field] = value
        }
      }
    ...
    formatPriceInput(event, field, type, index = null) {
      ...
      // Update the model with numeric value (tanpa titik)
      if (type === 'room' && index !== null) {
        if (this.showAddGuesthouseModal || this.showAddRoomModal) {
          this.newRoom.pricing[index][field] = numericValue
        } else if (this.showEditGuesthouseModal && this.selectedEditRoom) {
          this.selectedEditRoom.pricing[index][field] = numericValue
        }
      }
    ...
    // Get formatted price for display (dengan titik pemisah ribuan)
    ...
    async fetchGuesthouses() {
      ...
      try {
        // fetch gedung mess menggunakan fungsi apiService.js
        const response = await apiService.getGuesthouses(token)

        if (response.data.status) {
          this.guesthouses = response.data.data
        } else {
          this.showToast('error', 'Gagal mengambil data ruang mess.')
        }
      } catch (error) {
    ...
    resetForms() {
      ...
      // Reset room form
      this.newRooms = [] // Reset array ruang
    ...
    goBackToStep1() {
      ...
      // Reinitialize map saat kembali ke step 1
      this.$nextTick(() => {
        // Tunggu DOM update
        setTimeout(() => {
          // Destroy existing map instance jika ada
          if (this.map) {
            try {
              this.map.remove()
              this.map = null
    ...
    handleRoomPhotoChange(event) {
      ...
      // Add new files to the array - EXACTLY like guesthouse
      for (let i = 0; i < files.length; i++) {
        const file = files[i]
        this.roomPhotos.push(file)

        // Store metadata for newRoom.media
        this.newRoom.media.push({
          file,
          name: file.name,
          size: file.size,
          type: file.type,
    ...
    removePricing(index) {
      if (this.newRoom.pricing.length > 1) {
        ...
        this.showToast('warning', 'Minimal harus ada satu kategori harga sewa')
      }
    },

    // Room management untuk tambah gedung mess
    resetRoomForm() {
      this.currentRoomIndex = null
    ...
    // Methods untuk modal tambah/edit ruang (konsep seperti Edit Gedung Mess)
    openAddNewRoomModal() {
      this.selectedAddRoomIndex = null
      this.resetRoomForm()
    ...
    submitAddRoomModalForm() {
      ...
      if (!this.newRoom.name.trim()) {
        this.showToast('warning', 'Nama ruang harus diisi')
        return
      }

      if (this.newRoom.facilitiesList.length === 0) {
        this.showToast('warning', 'Silakan tambahkan minimal satu fasilitas ruang')
        return
      }

      if (!this.newRoom.total_slot || parseInt(this.newRoom.total_slot) <= 0) {
        this.showToast('warning', 'Silakan isi total slot dengan angka yang valid')
        return
      }

      if (!this.newRoom.area_m2 || parseInt(this.newRoom.area_m2) <= 0) {
        this.showToast('warning', 'Silakan isi luas ruangan dengan angka yang valid')
        return
      }

      if (
        this.newRoom.pricing.some(
          (p) => !p.retribution_type.trim() || !p.price_per_day || parseInt(p.price_per_day) <= 0,
        )
      ) {
        this.showToast('warning', 'Silakan lengkapi semua kategori dan harga sewa')
        return
      }

      // Clone room data dengan photos
      const roomData = {
        ...JSON.parse(JSON.stringify(this.newRoom)),
        photos: [...this.roomPhotos],
        photoPreviews: [...this.roomPhotoPreviews],
      }
    ...
    async submitGuesthouseForm() {
      ...
      if (this.newGuesthouse.description.length < 10) {
        this.showToast('warning', 'Tentang gedung harus minimal 10 karakter')
        return
      }

      // Validate address length
      if (!this.newGuesthouse.address || this.newGuesthouse.address.trim().length < 5) {
        this.showToast('warning', 'Alamat harus minimal 5 karakter')
        return
      }

      // Validate facilities - minimal 2 fasilitas
      if (this.newGuesthouse.facilitiesList.length < 2) {
        this.showToast('warning', 'Silakan tambahkan minimal 2 fasilitas utama')
        return
      }

      if (this.guesthousePhotos.length === 0) {
        this.showToast('warning', 'Silakan upload minimal satu foto gedung')
        return
      }

      // Validate numeric fields
      if (!this.newGuesthouse.area_m2 || this.newGuesthouse.area_m2 === '0') {
        this.showToast('warning', 'Silakan isi luas bangunan dengan angka yang valid')
        return
      }

      if (!this.newGuesthouse.contact_person) {
        this.showToast('warning', 'Silakan isi contact person dengan nomor telepon yang valid')
        return
      }

      // Validate map coordinates
      if (!this.newGuesthouse.latitude || !this.newGuesthouse.longitude) {
        this.showToast('warning', 'Silakan pilih lokasi pada peta')
        return
      }
    ...
    // Form submission - Room (submit semua ruang yang sudah ditambahkan)
    async submitRoomForm() {
      // Validasi: minimal harus ada 1 ruang
      if (this.newRooms.length === 0) {
        this.showToast('warning', 'Silakan tambahkan minimal satu ruang mess')
        return
      }
      ...
      try {
        ...
        guesthouseFormData.append('area_m2', parseInt(this.newGuesthouse.area_m2))
        ...
        console.log('=== Guesthouse FormData ===')
        for (let pair of guesthouseFormData.entries()) {
          console.log(pair[0] + ':', pair[1])
        }

        // submit form gedung mess menggunakan fungsi dari apiService.js
        ...
        // STEP 2: Submit semua ruang yang ada di array
        let successCount = 0
        let failCount = 0

        for (let i = 0; i < this.newRooms.length; i++) {
          try {
            const room = this.newRooms[i]
            ...
            // Convert pricing data - ensure price_per_day is number
            const pricingData = room.pricing.map((p) => ({
              retribution_type: p.retribution_type,
              price_per_day: parseInt(p.price_per_day),
            ...
            // Debug: Log Room FormData content
            console.log(`=== Room ${i + 1} FormData ===`)
            for (let pair of roomFormData.entries()) {
              console.log(pair[0] + ':', pair[1])
            }

            // submit form ruang mess menggunakan fungsi dari apiService.js
            const roomResponse = await apiService.createGuesthouseRoom(
              createdGuesthouseId,
              roomFormData,
            )
            ...
            if (roomResponse.data.status) {
              ...
              failCount++
              console.error(`Failed to add room ${room.name}:`, roomResponse.data.message)
            }
          } catch (error) {
            failCount++
            console.error(`Error adding room ${i + 1}:`, error)
          }
        }
        ...
        if (successCount > 0 && failCount === 0) {
          ...
          this.showToast('success', `Gedung mess dan ${successCount} ruang berhasil ditambahkan!`)
        } else if (successCount > 0 && failCount > 0) {
          this.closeAddGuesthouseModal()
          this.fetchGuesthouses()
          this.showToast(
            'warning',
            `Gedung mess dan ${successCount} ruang berhasil ditambahkan, ${failCount} ruang gagal ditambahkan.`,
          )
        } else {
          throw new Error('Semua ruang gagal ditambahkan')
        }
      } catch (error) {
        console.error('Error creating guesthouse and rooms:', error)
        ...
        this.showToast('error', `Gagal menambahkan gedung mess: ${errorMessage}`)
      } finally {
    ...
    async fetchRoomData(guesthouseId) {
      ...
      try {
        ...
        try {
          ...
          // fetch ruang mess menggunakan fungsi dari apiService.js
          ...
          try {
            ...
            if (response.data.status) {
              ...
              if (!roomData.guesthouse_id || roomData.guesthouse_id == guesthouseId) {
                ...
                console.warn(
                  `Room ${roomId} belongs to guesthouse ${roomData.guesthouse_id}, not ${guesthouseId}`,
                )
    ...
    async fetchGuesthouseDetailData(id) {
      ...
      try {
        // fetch gedung mess menggunakan fungsi dari apiService.js
        ...
        if (response.data.status) {
          ...
          this.showToast('error', 'Gagal mengambil detail ruang mess.')
          this.showModal = false
        }
      } catch (error) {
        console.error('Error fetching guesthouse detail:', error)
        this.showToast('error', 'Terjadi kesalahan saat mengambil detail ruang mess.')
        this.showModal = false
      } finally {
        this.isLoadingDetail = false
      }
      ...
      try {
        ...
        for (const room of this.rooms) {
          ...
          if (room.guesthouse_id && parseInt(room.guesthouse_id) !== parseInt(guesthouseId)) {
            console.warn(
              `Skipping reviews for room ${room.id} - belongs to guesthouse ${room.guesthouse_id}, not ${guesthouseId}`,
            )
            continue
          }

          try {
            // fetch ulasan menggunakan fungsi dari apiService.js
            const response = await apiService.getReviewsForGuesthouseRoom(
              guesthouseId,
              room.id,
              token,
            )
            ...
            if (
              ...
              // Tambahkan info room ke setiap review dan normalisasi review_reply
              const roomReviews = response.data.data.map((review) => {
                ...
                // Normalize review_reply if it exists
                if (review.review_reply) {
                  normalizedReview.review_reply = {
    ...
    async fetchAdminData(adminId) {
      ...
      try {
        ...
        const response = await apiService.getAdmin(adminId, token) // PERBAIKAN: gunakan adminId bukan id
    ...
    async submitReply(rentId, reviewId) {
      ...
      try {
        // submit balasan menggunakan fungsi dari apiService.js
    ...
    async deleteReply(rentId, reviewId) {
      ...
      try {
        // delete balasan menggunakan fungsi dari apiService.js
    ...
    proceedToEditStep2() {
      // Validate form
      if (this.editGuesthouse.facilitiesList.length < 2) {
        this.showToast('warning', 'Silakan tambahkan minimal 2 fasilitas utama')
        return
      }
    ...
    async loadGuesthouseForEdit(guesthouseId) {
      ...
      try {
        // fetch detail gedung mess menggunakan fungsi dari apiService.js
        ...
        if (response.data.status) {
          ...
          this.editGuesthouse = {
            ...
            facilitiesList: guesthouse.facilities
              ? guesthouse.facilities.split(';').map((item) => item.trim())
              : [],
          ...
          this.editGuesthousePhotoPreviews = (guesthouse.guesthouse_media || []).map(
            (media) => media.url,
          )
          ...
          // Backup data asli untuk restore jika cancel
          this.originalEditRooms = JSON.parse(JSON.stringify(this.editRooms))

          // Reset tracking state
          this.hasUnsavedChanges = false
          this.pendingDeletedRooms = []
          this.pendingEditedRooms = []
          this.pendingNewRooms = []
          ...
          this.showToast('error', 'Gagal mengambil detail ruang mess untuk pengeditan.')
          this.showEditGuesthouseModal = false
        }
      } catch (error) {
        console.error('Error fetching guesthouse for edit:', error)
        this.showToast('error', 'Terjadi kesalahan saat mengambil data gedung mess.')
        this.showEditGuesthouseModal = false
      }
    },
        ...
        while (consecutiveErrors < maxConsecutiveErrors) {
          try {
            ...
            if (response.data.status) {
              ...
              if (
                !roomData.guesthouse_id ||
                parseInt(roomData.guesthouse_id) == parseInt(guesthouseId)
              ) {
                roomsData.push(roomData)
                consecutiveErrors = 0
              } else {
    ...
    // Konfirmasi batalkan perubahan
    confirmCancelChanges() {
      this.showCancelConfirmModal = false
      this.performCloseEditModal()
    },

    // Fungsi untuk menutup modal edit (dipanggil setelah konfirmasi atau jika tidak ada perubahan)
    performCloseEditModal() {
      // Restore data asli jika ada perubahan yang dibatalkan
      if (this.hasUnsavedChanges) {
        this.editRooms = JSON.parse(JSON.stringify(this.originalEditRooms))
      }
    ...
    addEditRoomFacility() {
      ...
      if (facility) {
        this.editRoom.facilitiesList.push(facility)
    ...
    async submitEditGuesthouseForm() {
      // Validate description length
      if (this.editGuesthouse.description.length < 10) {
        this.showToast('warning', 'Tentang gedung harus minimal 10 karakter')
        return
      }

      // Validate address length
      if (!this.editGuesthouse.address || this.editGuesthouse.address.trim().length < 5) {
        this.showToast('warning', 'Alamat harus minimal 5 karakter')
        return
      }

      // Validate form - minimal 2 fasilitas
      if (this.editGuesthouse.facilitiesList.length < 2) {
        this.showToast('warning', 'Silakan tambahkan minimal 2 fasilitas utama')
        return
      }
      ...
      // Validate numeric fields
      if (!this.editGuesthouse.area_m2 || this.editGuesthouse.area_m2 === '0') {
        this.showToast('warning', 'Silakan isi luas bangunan dengan angka yang valid')
        return
      }

      if (!this.editGuesthouse.contact_person) {
        this.showToast('warning', 'Silakan isi contact person dengan nomor telepon yang valid')
        return
      }
      ...
      try {
        ...
        // Submit guesthouse update
        const guesthouseResponse = await apiService.updateGuesthouse(
          this.editGuesthouseId,
          formData,
          token,
        )

        if (!guesthouseResponse.data.status) {
          throw new Error(guesthouseResponse.data.message || 'Failed to update guesthouse')
        }

        // STEP 2: Process pending deleted rooms
        for (const roomId of this.pendingDeletedRooms) {
          try {
            await apiService.deleteGuesthouseRoom(this.editGuesthouseId, roomId, token)
          } catch (error) {
            console.error(`Error deleting room ${roomId}:`, error)
          }
        }

        // STEP 3: Process pending new rooms
        for (const newRoom of this.pendingNewRooms) {
          try {
            const formattedPricing = newRoom.pricing.map((p) => ({
              id: 0,
              retribution_type: p.retribution_type,
            ...
            // Note: photos untuk new room harus di-handle terpisah jika ada
        ...
        for (const roomId of this.pendingEditedRooms) {
          try {
            ...
            await apiService.updateGuesthouseRoom(
              this.editGuesthouseId,
              roomId,
              roomFormData,
              token,
        ...
        this.showToast(
          'error',
          `Gagal memperbarui gedung mess: ${error.message || 'Terjadi kesalahan'}`,
        )
      } finally {
    ...
    // Submit edited room (state sementara, belum ke API)
    async submitEditRoomForm() {
      ...
      // Update room in editRooms array (state sementara)
      const roomIndex = this.editRooms.findIndex((room) => room.id === this.editRoom.id)
      if (roomIndex !== -1) {
        this.editRooms[roomIndex] = { ...this.editRoom }
      }

      // Tandai room sebagai pending edit jika belum ada
      if (!this.pendingEditedRooms.includes(this.editRoom.id)) {
        this.pendingEditedRooms.push(this.editRoom.id)
      }
      ...
      this.showToast(
        'success',
        'Perubahan ruang disimpan sementara. Tekan "Simpan Perubahan" untuk menyimpan.',
      )
    },

    // Submit new room (state sementara, belum ke API)
    async submitNewRoomForm() {
      ...
      // Generate temporary ID untuk ruang baru (gunakan timestamp negatif)
      const tempId = -Date.now()
      const newRoom = {
        ...this.editRoom,
        id: tempId,
        isNew: true, // Flag untuk identifikasi ruang baru
      }

      // Add the new room to the editRooms array (state sementara)
      this.editRooms.push(newRoom)

      // Tambahkan ke pending new rooms
      ...
      this.showToast(
        'success',
        'Ruang baru ditambahkan sementara. Tekan "Simpan Perubahan" untuk menyimpan.',
      )
    },

    // Delete a room - Buka modal konfirmasi
    deleteRoom(room) {
      this.roomToDelete = room
      this.showDeleteRoomModal = true
    },

    // Tutup modal konfirmasi hapus ruang
    closeDeleteRoomModal() {
      this.showDeleteRoomModal = false
      this.roomToDelete = null
    },

    // Konfirmasi hapus ruang (state sementara, belum ke API)
    async confirmDeleteRoom() {
      if (!this.roomToDelete) return

      // Tandai sebagai pending delete (state sementara)
      if (!this.pendingDeletedRooms.includes(this.roomToDelete.id)) {
        this.pendingDeletedRooms.push(this.roomToDelete.id)
      }

      // Remove from editRooms array (state sementara)
      this.editRooms = this.editRooms.filter((room) => room.id !== this.roomToDelete.id)

      // If the room being edited is the one deleted, close the edit room form
      if (this.editRoom.id === this.roomToDelete.id) {
        this.closeEditRoomForm()
      }
      ...
      this.showToast(
        'success',
        'Ruang ditandai untuk dihapus. Tekan "Simpan Perubahan" untuk menyimpan.',
      )
      this.closeDeleteRoomModal()
    },

    // Open modal konfirmasi hapus gedung mess
    openDeleteGuesthouseModal(guesthouse) {
      console.log('openDeleteGuesthouseModal called with:', guesthouse)
      this.guesthouseToDelete = guesthouse
      this.showDeleteGuesthouseModal = true
      console.log('showDeleteGuesthouseModal set to:', this.showDeleteGuesthouseModal)
    },

    // Close modal konfirmasi hapus gedung mess
    closeDeleteGuesthouseModal() {
      this.showDeleteGuesthouseModal = false
      this.guesthouseToDelete = null
    },

    // Konfirmasi dan hapus gedung mess
    async confirmDeleteGuesthouse() {
      if (!this.guesthouseToDelete) return

      this.isDeletingGuesthouse = true
      this.deletingGuesthouseId = this.guesthouseToDelete.id
      ...
      try {
        // delete gedung mess menggunakan fungsi dari apiService.js
        ...
        if (response.data.status) {
          ...
          this.guesthouses = this.guesthouses.filter(
            (guesthouse) => guesthouse.id !== this.guesthouseToDelete.id,
          )

          this.showToast('success', 'Gedung mess berhasil dihapus!')
          this.closeDeleteGuesthouseModal()
        } else {
        ...
        this.showToast(
          'error',
          `Gagal menghapus gedung mess: ${error.response?.data?.message || error.message || 'Terjadi kesalahan'}`,
        )
      } finally {
        this.isDeletingGuesthouse = false
        this.deletingGuesthouseId = null
    ...
    // Delete a guesthouse (Legacy - kept for compatibility, now just opens modal)
    async deleteGuesthouse(guesthouseId) {
      // Find the guesthouse object
      const guesthouse = this.guesthouses.find((g) => g.id === guesthouseId)
      if (guesthouse) {
        this.openDeleteGuesthouseModal(guesthouse)
      }
    },
