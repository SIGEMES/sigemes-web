...
export default {
  ...
  computed: {
    ...
    paginatedRentals() {
      // If we're using server-side pagination (rentals contains only the current page),
      // return rentals directly. Otherwise fall back to client-side slicing.
      if (this.totalData && this.totalData > this.rentals.length) {
        return this.rentals
      }
  ...
  methods: {
    ...
    handlePageChange(newPage) {
      ...
      // Fetch rentals for the newly selected page (server-side pagination)
      this.fetchRentals()
    },
    ...
    async fetchRentals() {
      ...
      try {
        ...
        if (response.data.status) {
          ...
          // If totalPages still unset (no meta), compute from fetched length
          if (!this.totalPages) {
