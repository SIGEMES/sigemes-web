  ...
  methods: {
    ...
    async fetchRentals() {
      ...
      try {
        ...
        // 1. Gunakan currentPage dan itemsPerPage yang sedang aktif
        ...
        // PENTING: Jika backend mendukung filter status dan tipe, kirimkan juga lewat params
        ...
        if (response.data.status) {
          // 2. Terapkan data HANYA untuk halaman ini
          this.rentals = response.data.data;
          
          // 3. Ambil metadata total halaman dari backend (Sesuaikan dengan struktur response API kamu)
          // Asumsi backend mereturn response.data.pagination: { total, last_page, current_page }
          if (response.data.pagination) {
            this.totalData = response.data.pagination.total;
            this.totalPages = response.data.pagination.last_page;
          } else {
            // Fallback jika API tidak memberikan objek pagination eksplisit
            this.totalData = response.data.total || this.rentals.length;
            this.totalPages = response.data.last_page || Math.ceil(this.totalData / this.itemsPerPage);
          }

          // 4. Fetch Renter Detail sekarang HANYA akan me-looping 10 data, bukan 1000 data!
    ...
    goToPage(page) {
      if (page >= 1 && page <= this.totalPages && page !== this.currentPage) {
        ...
        this.fetchRentals(); // <-- Tambahkan ini untuk menarik data halaman baru
      }
    },
