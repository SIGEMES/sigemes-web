...
export default {
  ...
  data() {
    return {
      ...
      currentMonthText: '',
      currentYear: '',
  // Cache renter names by id to avoid repeated network calls
  renterCache: {},
  // Whether we're using server-side pagination (backend returns single page + meta)
  ...
  computed: {
    ...
    paginatedRentals() {
      // If we're using server-side pagination (backend returns only current page), return rentals directly.
      if (this.serverSidePagination) {
        return this.rentals
      }

      // Otherwise fall back to client-side slicing (we have full dataset locally)
  ...
  methods: {
    ...
    async fetchRentals() {
      // Use server-side pagination to reduce payload and speed up requests.
      // We request only the current page and itemsPerPage, and rely on the API to apply filters/search when possible.
      this.isLoading = true
      this.errorMessage = ''
      ...
      try {
        ...
        // Add search parameter (let backend handle search if supported)
        if (this.searchQuery) {
        ...
        // Add rental type & payment status filters if present (let backend filter)
        if (this.appliedFilterRentalType) params.append('rental_type', this.appliedFilterRentalType)
        if (this.appliedSelectedPaymentStatuses && this.appliedSelectedPaymentStatuses.length) {
          // send as comma separated list
          params.append('payment_statuses', this.appliedSelectedPaymentStatuses.join(','))
        }

        // fetch rentals for current page using apiService.js
        const response = await apiService.getRentals(params)

        if (response.data.status) {
          // Prefer server-provided data array
          const fetched = response.data.data || []

          // Attempt to detect server-side pagination via meta or headers
          const meta = response.data.meta || {}
          const headers = response.headers || {}

          // Determine total pages robustly from several possible shapes
          let totalPagesFromServer = null
          let totalCount = null

          if (meta.last_page) {
            // When API returns last_page (common in Laravel pagination)
            totalPagesFromServer = Number(meta.last_page)
            totalCount = meta.total || null
          } else if (meta.total) {
            totalCount = Number(meta.total)
          } else if (response.data.total) {
            totalCount = Number(response.data.total)
          } else if (headers['x-total-count']) {
            totalCount = Number(headers['x-total-count'])
          }

          // If last_page provided, use it. Otherwise compute from totalCount when available.
          if (totalPagesFromServer) {
            this.totalPages = Math.max(1, totalPagesFromServer)
            this.serverSidePagination = true
          } else if (totalCount !== null) {
            this.totalData = totalCount
            this.totalPages = Math.max(1, Math.ceil(totalCount / (this.itemsPerPage || 10)))
            this.serverSidePagination = true
          } else {
            // Fallback: no server pagination info available -> treat as client-side (we received full list)
            this.serverSidePagination = false
          }

          // Set rentals to fetched page (or full list)
          this.rentals = fetched

          // If we didn't set totalData above but server didn't provide it, set fallback
          if (!this.totalData) this.totalData = fetched.length

          // If totalPages still unset (no meta/total), attempt a lightweight probe to detect next page
          if (!this.totalPages) {
            try {
              const detected = await this.determineTotalPages(params)
              if (detected && detected > 0) {
                this.totalPages = detected
                this.serverSidePagination = true
              } else {
                // Fallback
                this.totalPages = Math.max(1, Math.ceil(fetched.length / (this.itemsPerPage || 10)))
                this.serverSidePagination = false
              }
            } catch (err) {
              console.error('Failed to determine total pages:', err)
              this.totalPages = Math.max(1, Math.ceil(fetched.length / (this.itemsPerPage || 10)))
              this.serverSidePagination = false
            }
          }
          ...
          // Only fetch renter details for the current page (unique ids) to reduce requests
    ...
    async fetchRenterDetails() {
      // Only fetch renter details for rentals on the current page and cache results.
      if (!this.rentals || this.rentals.length === 0) return

      // Collect unique renter IDs from current rentals page
      const renterIds = Array.from(
        new Set(this.rentals.map((r) => r.renter_id).filter(Boolean))
      )

      // Determine which IDs we still need to fetch (not in cache)
      const idsToFetch = renterIds.filter((id) => !this.renterCache[id])

      // Fetch missing renters in parallel (batched)
      const batchPromises = idsToFetch.map(async (id) => {
        try {
          const response = await apiService.getRenter(id)
          if (response.data?.status) {
            this.renterCache[id] = response.data.data.fullname || 'Tidak ditemukan'
          } else {
            this.renterCache[id] = 'Tidak ditemukan'
          }
        } catch (err) {
          console.error(`Error fetching renter ${id}:`, err)
          this.renterCache[id] = 'Tidak ditemukan'
        }
      })
      ...
      // Map cached names back to rental items
      this.rentals.forEach((rental) => {
        if (rental.renter_id) {
          rental.renterName = this.renterCache[rental.renter_id] || 'Tidak ditemukan'
        } else {
          rental.renterName = rental.renterName || '-' // preserve if already set
        }
      })
    ...
    // Determine total pages by probing the backend with limit=1 using exponential + binary search.
    // This avoids loading all items while still discovering the exact number of pages.
    async determineTotalPages(baseParams) {
      // baseParams is a URLSearchParams instance
      const probeLimit = 1

      const getPageHasData = async (page) => {
        try {
          const p = new URLSearchParams(baseParams.toString())
          p.set('page', page)
          p.set('limit', probeLimit)
          const resp = await apiService.getRentals(p)
          return !!(resp?.data?.status && resp.data.data && resp.data.data.length > 0)
        } catch (e) {
          console.error('Error probing page', page, e)
          // If probe fails, treat as no data to avoid infinite loops
          return false
        }
      }

      // Quick check page 1
      const page1Has = await getPageHasData(1)
      if (!page1Has) return 0

      // Exponential search to find an upper bound where page has no data
      let low = 1
      let high = 2
      const MAX_PAGES_CAP = 1000 // safety cap to avoid runaway when backend ignores paging
      while (true) {
        // If we've exceeded a reasonable cap, abort probing and return 0 (unknown)
        if (high > MAX_PAGES_CAP) {
          console.warn('determineTotalPages: exceeded MAX_PAGES_CAP, aborting probe')
          return 0
        }

        const has = await getPageHasData(high)
        if (has) {
          low = high
          high = high * 2
        } else {
      ...
      // Binary search between low and high to find last page with data
      let left = low
      let right = high
      while (left < right) {
        const mid = Math.floor((left + right + 1) / 2)
        const hasMid = await getPageHasData(mid)
        if (hasMid) {
          left = mid
        } else {
          right = mid - 1
    ...
    async showRentalDetail(rentalId) {
      ...
      try {
        ...
        // fetch detail sewa berdasarkan id menggunakan fungsi apiService.js
        ...
        this.errorMessage =
          ...
          error.message ||
          'Gagal mengambil detail. Coba lagi nanti.'
    ...
    openPageJumpModal() {
      ...
      this.pageJumpError = '' // Reset error saat modal dibuka
    },
    closePageJumpModal() {
      this.showPageJumpModal = false
      this.pageJumpInput = null
      this.pageJumpError = '' // Reset error saat modal ditutup
    },
    validatePageInput() {
      ...
      if (!this.pageJumpInput) {
        ...
        this.pageJumpError = '' // Clear error jika valid
      }
    },
    jumpToPage() {
    ...
    goToPage(page) {
      if (page >= 1 && page <= this.totalPages && page !== this.currentPage) {
        ...
        // If using server-side pagination, fetch the new page immediately
        if (this.serverSidePagination) {
          this.fetchRentals()
        }
      }
