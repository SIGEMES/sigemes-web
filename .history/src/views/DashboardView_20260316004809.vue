    ...
    async fetchRentals() {
      ...
      try {
        ...
        if (response.data.status) {
          ...
          // Determine total count from common meta shapes, fallback to fetched length
          const totalFromMeta = response.data.meta?.total || response.data.total || fetched.length

          this.rentals = fetched
          this.totalData = Number(totalFromMeta) || fetched.length
          this.totalPages = Math.max(1, Math.ceil(this.totalData / (this.itemsPerPage || 10)))
