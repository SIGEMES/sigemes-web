    ...
    async fetchRentals() {
      ...
      try {
        ...
        if (response.data.status) {
          ...
          if (!this.totalPages) {
            // Do a probe: request page = currentPage + 1 with limit = 1 to see if there are more items
            try {
              const probeParams = new URLSearchParams(params.toString())
              probeParams.set('page', Number(this.currentPage || 1) + 1)
              probeParams.set('limit', 1)
              const probeResponse = await apiService.getRentals(probeParams)
              const hasNext = probeResponse.data?.data && probeResponse.data.data.length > 0

              if (hasNext) {
                // We don't know total yet, but allow one more page navigation
                this.totalPages = Number(this.currentPage || 1) + 1
                this.serverSidePagination = true
              } else {
                // No next page -> currentPage is last
                this.totalPages = Math.max(1, Math.ceil(fetched.length / (this.itemsPerPage || 10)))
                this.serverSidePagination = true
              }
            } catch (probeErr) {
              // If probe fails, fallback to treating fetched as full list
              console.error('Probe for next page failed:', probeErr)
              this.totalPages = Math.max(1, Math.ceil(fetched.length / (this.itemsPerPage || 10)))
              this.serverSidePagination = false
            }
          }
